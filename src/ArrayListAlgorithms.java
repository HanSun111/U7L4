...
public class ArrayListAlgorithms {
    ...
    { for(int i = 0; i < wordList.size(); i++) {
        if (wordList.indexOf(wordList.get(i).toLowerCase()) == i){
    ...
    public static void moveBWords(ArrayList<String> wordList)
    {
        ...
        for (int i = 0; i < wordList.size(); i++) {
            if (wordList.get(i).contains("b")){
                wordList.add(count, wordList.remove(i));
                count++;
            }
        }
    }
    ...
    public static ArrayList<Integer> modes(int[] numList)
    {
        ...
        for (int i = 0; i < numList.length; i++) {
            ...
            if (count > highestCount && count > 1 && !temp.contains(current)){
                temp.clear();
                temp.add(current);
                highestCount = count;
            } else if (count == highestCount && !temp.contains(current)) {
                temp.add(current);
            }
            count = 0;
        }
